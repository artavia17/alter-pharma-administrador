...
import { bulkCreateStates, type BulkStateData } from "../../services/protected/states.services";
import { CountryData } from "../../types/services/protected/countries.types";
import * as XLSX from 'xlsx';

interface BulkUploadStateModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
}

export default function BulkUploadStateModal({ isOpen, onClose, onSuccess }: BulkUploadStateModalProps) {
  const [countries, setCountries] = useState<CountryData[]>([]);
  const [selectedCountryId, setSelectedCountryId] = useState<number | null>(null);

  const [file, setFile] = useState<File | null>(null);
  const [previewData, setPreviewData] = useState<BulkStateData[]>([]);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const fileInput = document.getElementById('state-excel-upload') as HTMLInputElement;
      if (fileInput) {
        fileInput.value = '';
      }
    }
  ...
  const parseExcelFile = (file: File) => {
    if (!selectedCountryId) {
      setErrors(['Debes seleccionar un país antes de cargar el archivo']);
      return;
    }
    ...
    reader.onload = (e) => {
      try {
        ...
        const parsedData: BulkStateData[] = jsonData.map((row: any) => ({
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < totalBatches; i++) {
      ...
      try {
        const response = await bulkCreateStates({ states: batch });
  ...
  const downloadTemplate = () => {
    ...
    XLSX.writeFile(wb, 'plantilla_ciudades_provincias.xlsx');
  };
      ...
      <div className="relative w-full max-w-4xl rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-8">
        <div className="px-2 pr-14 mb-6">
          <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
            Carga Masiva de Ciudades/Provincias
          </h4>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Carga múltiples ciudades o provincias desde un archivo Excel
          </p>
        </div>
        ...
        {showResults && (
          <div className="px-2 mb-4 space-y-3">
            <Alert
              ...
              message={`Se crearon ${successCount} ciudades/provincias exitosamente. ${failedCount > 0 ? `${failedCount} fallaron.` : ''}`}
            />
            {errors.length > 0 && (
        ...
        <div className="px-2 space-y-6">
          ...
          <div>
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                Todas las ciudades/provincias se crearán en el país seleccionado
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
              <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
                Descarga la plantilla Excel con el formato correcto para cargar las ciudades/provincias:
              </p>
              <ul className="text-sm text-gray-600 dark:text-gray-400 mb-3 space-y-1 list-disc list-inside">
                <li><strong>Nombre:</strong> Nombre de la ciudad o provincia</li>
                <li><strong>Código:</strong> Código o abreviatura (ej: SJ, AL, CA)</li>
              </ul>
            ...
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 dark:border-gray-700 rounded-lg p-6 text-center">
                <input
                  ...
                  className="hidden"
                  id="state-excel-upload"
                  disabled={isUploading}
                />
                <label htmlFor="state-excel-upload" className="cursor-pointer">
          ...
          {previewData.length > 0 && !isUploading && !showResults && (
            <div>
              <h5 className="text-lg font-medium text-gray-800 dark:text-white/90 mb-4">
                Vista previa ({previewData.length} ciudades/provincias)
              </h5>
              <div className="max-h-64 overflow-y-auto border border-gray-200 dark:border-gray-700 rounded-lg">
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  <thead className="bg-gray-50 dark:bg-gray-800">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">País</th>
                    </tr>
                  </thead>
                    ...
                    {previewData.slice(0, 10).map((item, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-900 dark:text-gray-100">{country?.name}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                {previewData.length > 10 && (
                  <div className="px-4 py-3 bg-gray-50 dark:bg-gray-800 text-sm text-gray-500 dark:text-gray-400">
                    Mostrando 10 de {previewData.length} ciudades/provincias
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
          ...
          {previewData.length > 0 && !showResults && (
            <Button
              ...
              disabled={isUploading || !selectedCountryId}
            >
              {isUploading ? 'Procesando...' : `Cargar ${previewData.length} Ciudades/Provincias`}
            </Button>
          )}
        </div>
      </div>
