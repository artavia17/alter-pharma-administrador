...
import { getDistributors, createDistributor, updateDistributor, updateDistributorCredentials, toggleDistributorStatus, deleteDistributor } from "../../../services/protected/distributors.services";
...
export default function DistribuidoresPage() {
  ...
  const [credTarget, setCredTarget] = useState<DistributorData | null>(null);
  ...
  const openCred = (distributor: DistributorData) => {
    setCredTarget(distributor);
    setCredEmail(distributor.email);
    setCredPassword("");
    setCredErrors({});
    openCredModal();
  };
  ...
  const handleUpdateCredentials = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await updateDistributorCredentials(credTarget.id, params);
      if (response.status === 200) {
        await loadDistributors();
        handleCloseCred();
      }
                  ...
                  <TableRow key={distributor.id}>
                    ...
                    <TableCell className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => openCred(distributor)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg dark:text-green-400 dark:hover:bg-green-900/20" title="Actualizar credenciales">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
      ...
      <Modal isOpen={isCredOpen} onClose={handleCloseCred} className="max-w-[450px] m-4">
        <div className="no-scrollbar relative w-full max-w-[450px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-8">
          <div className="px-2 pr-14 mb-6">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">{credTarget?.business_name}</p>
          </div>
          {credErrors.general && (
            <div className="px-2 mb-4">
