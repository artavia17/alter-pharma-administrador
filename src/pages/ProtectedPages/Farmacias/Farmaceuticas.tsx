...
import { getPharmacies, createPharmacy, updatePharmacy, updatePharmacyCredentials, togglePharmacyStatus } from "../../../services/protected/pharmacies.services";
...
export default function FarmaceuticasPage() {
  ...
  const [credTarget, setCredTarget] = useState<PharmacyData | null>(null);
  ...
  const openCred = (pharmacy: PharmacyData) => {
    setCredTarget(pharmacy);
    setCredEmail(pharmacy.email);
    setCredPassword("");
    setCredErrors({});
    openCredModal();
  };
  ...
  const handleUpdateCredentials = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await updatePharmacyCredentials(credTarget.id, params);
      if (response.status === 200) {
        await loadPharmacies();
        handleCloseCred();
      }
                  ...
                  <TableRow key={pharmacy.id}>
                    ...
                    <TableCell className="px-5 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <button onClick={() => openCred(pharmacy)} className="p-2 text-green-600 hover:bg-green-50 rounded-lg dark:text-green-400 dark:hover:bg-green-900/20" title="Actualizar credenciales">
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
