...
export const getReversedRedemptionTransactions = async (params?: {
  search?: string;
  from?: string;
  to?: string;
  per_page?: number;
  ...
  if (params?.page) q.append('page', params.page.toString());
  const qs = q.toString();
  const response = await api.get(qs ? `/administrator/redemption-transactions/reversed?${qs}` : '/administrator/redemption-transactions/reversed');
  return response.data;
};

export const restoreRedemptionTransaction = async (id: number) => {
  const response = await api.post(`/administrator/redemption-transactions/${id}/restore`);
  return response.data;
};
